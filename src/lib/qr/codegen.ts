...
namespace qrcodegen {
  ...
  export class QrCode {
    ...
    private setFunctionModule(x: int, y: int, isDark: boolean): void {
      if (this.modules[y] && this.isFunction[y]) {
    ...
    private addEccAndInterleave(data: Readonly<Array<byte>>): Array<byte> {
      ...
      const numBlocks: int =
        QrCode.NUM_ERROR_CORRECTION_BLOCKS[ecl.ordinal]?.[ver] ?? 0; // Provide a default value if undefined
      const blockEccLen: int =
        QrCode.ECC_CODEWORDS_PER_BLOCK[ecl.ordinal]?.[ver] ?? 0; // Provide a default value if undefined
    ...
    private applyMask(mask: int): void {
      ...
      for (let y = 0; y < this.size; y++) {
        for (let x = 0; x < this.size; x++) {
          ...
          if (this.isFunction[y] && !this.isFunction[y]?.[x] && invert)
            if (this.modules[y]) {
    ...
    private getPenaltyScore(): number {
      ...
      for (let y = 0; y < this.size; y++) {
        ...
        for (let x = 0; x < this.size; x++) {
          if (this.modules[y]?.[x] === runColor) {
            ...
            runColor = this.modules[y]?.[x] ?? false;
            runX = 1;
          }
        }
        result +=
      ...
      for (let y = 0; y < this.size - 1; y++) {
        for (let x = 0; x < this.size - 1; x++) {
          const color = this.modules[y]?.[x] ?? false;
          if (
...
namespace qrcodegen.QrSegment {
  ...
  export class Mode {
    ...
    public numCharCountBits(ver: int): int {
      const index = Math.floor((ver + 7) / 17);
      return this.numBitsCharCount[index] ?? 0;
    }
  }
}
